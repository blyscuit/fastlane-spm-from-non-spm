...
*/
public func copyFiles(source: String,
                      destination: String) {
...
let array: [RubyCommand.Argument?] = [sourceArg,
destinationArg]
let args: [RubyCommand.Argument] = array
.filter { $0?.value != nil }
.compactMap { $0 }
let command = RubyCommand(commandID: "", methodName: "copy_files", className: nil, args: args)
...
/**
 ...
 - parameter path: Path for files to remove
*/
public func removeFiles(path: String) {
let pathArg = RubyCommand.Argument(name: "path", value: path, type: nil)
let array: [RubyCommand.Argument?] = [pathArg]
let args: [RubyCommand.Argument] = array
.filter { $0?.value != nil }
.compactMap { $0 }
let command = RubyCommand(commandID: "", methodName: "remove_files", className: nil, args: args)
  _ = runner.executeCommand(command)
}
